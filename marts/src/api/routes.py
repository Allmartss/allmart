...
async def login(request: Request, user_data: UserCreate2, db: Session = Depends(get_db)):
    ...
    if _prefs_2fa.get("tfa_enabled"):
        ...
        _SECRET = os.getenv("MARTS_JWT_SECRET", "").strip()
...
async def send_verify_email(current_user=Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if wa_phone:
        try:
            from twilio.rest import Client as _Twilio
            _tc = _Twilio(os.getenv("MARTS_TWILIO_ACCOUNT_SID"), os.getenv("MARTS_TWILIO_AUTH_TOKEN"))
            _from = f"whatsapp:{os.getenv('MARTS_TWILIO_WHATSAPP_NUMBER', '+14155238886')}"
            _tc.messages.create(
                from_=_from,
...
async def admin_health_check(db: Session = Depends(get_db)):
    ...
    # ── Database ──────────────────────────────────────────────────────────────
    try:
        from src.database.session import MARTS_DATABASE_URL
        t0 = time.time()
        db.execute(__import__("sqlalchemy").text("SELECT 1"))
        ...
        checks["database"] = {
            "status": "healthy",
            "latency_ms": latency,
            "provider": "dedicated_marts" if MARTS_DATABASE_URL else "database_url_fallback",
        }
    except Exception as e:
        checks["database"] = {"status": "error", "error": str(e)}

    # ── Marts environment check ──────────────────────────────────────────────
    try:
        db_url_set = bool(os.getenv("MARTS_DATABASE_URL", "").strip())
        jwt_set = bool(os.getenv("MARTS_JWT_SECRET", "").strip())
        checks["marts_env"] = {
            "status": "healthy" if db_url_set and jwt_set else "degraded",
            "MARTS_DATABASE_URL": "set" if db_url_set else "using DATABASE_URL fallback",
            "MARTS_JWT_SECRET": "set" if jwt_set else "missing",
        }
    except Exception as e:
        checks["marts_env"] = {"status": "error", "error": str(e)}
    ...
    # ── Marts storage backend ─────────────────────────────────────────────────
    try:
        from src.utils.storage import (
            _s3_available,
            S3_KEY_ID, S3_ENDPOINT, S3_REGION, S3_BUCKET, FOLDER,
        )
        s3_ok  = _s3_available()
    ...
    # ── NewsAPI ───────────────────────────────────────────────────────────────
    try:
        news_key = os.getenv("MARTS_NEWSAPI_KEY", "").strip()
        if news_key:
            import httpx as _hx
            t0 = time.time()
