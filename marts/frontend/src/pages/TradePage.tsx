  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      try {
        ...
        const ws = new WebSocket(`${proto}//${window.location.host}/marts/ws/live?token=${encodeURIComponent(token)}`)
        wsRef.current = ws
...
function useTradeLivePrice(pair: string) {
  ...
  const fetch_ = useCallback(async () => {
    try {
      const res = await fetch('/marts/api/public/prices')
      if (!res.ok) return
...
function FinChatPanel({ pair, livePrice, liveChange, collapsed, onToggle, usingBalance, selExchange }: {
  ...
  const callAI = async (userText: string) => {
    ...
    setAiLoading(true)
    try {
      const res = await fetch('/marts/api/ai/chat', {
        method: 'POST',
