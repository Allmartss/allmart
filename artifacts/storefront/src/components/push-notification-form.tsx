...
import { Bell, Loader2, Users, User, Search, CheckSquare, Square, X } from "lucide-react";
...
export function PushNotificationForm() {
  ...
  async function handleSend(e: React.FormEvent) {
    ...
    setSending(true);
    try {
      const body: { title: string; message: string; userIds?: number[] } = { title, message };
  ...
  return (
    <Card className="p-6 border-border/50 shadow-sm">
      <div className="flex items-center gap-2 mb-4">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {mode === "all" ? "Sends to all registered users instantly." : "Sends to selected users only."}
          </p>
        </div>
      </div>
      ...
      {/* User picker */}
      {mode === "selected" && (
        <div className="mb-4 rounded-xl border border-border/60 overflow-hidden">
          {/* Search bar */}
